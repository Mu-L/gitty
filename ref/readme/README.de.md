...
Ungewöhnlich für andere Git-Browser:
...
- **Eine echte Shell, angedockt an die Historie.** Kein Widget, das git aufruft,
  ...
  Fenster wie das Diff. Die meisten Git-Browser lassen das Terminal draußen, also
  heißt eine Vermutung prüfen: Fenster wechseln. Hier ist es gleich da, und jeder
  andere Bereich aktualisiert sich, wenn sich das Repository ändert.
- **Zwei Commits auf einmal.** Einen anklicken, dann mit <kbd>Strg+Klick</kbd> /
  <kbd>Umschalt+Klick</kbd> einen zweiten, und das Paar an Ort und Stelle
  vergleichen — die meisten Browser vergleichen einen Commit nur mit seinem
  Vorgänger oder mit einem im Dialog gewählten Baum.
- **Jeden Branch lesen, ohne ihn auszuchecken.** Einen Branch wählen, und seine
  ganze Historie liegt zum Lesen bereit; Arbeitsverzeichnis, Diffs und Terminals
  bleiben genau dort, wo git sie gelassen hat. Im Arbeitsverzeichnis bewegt sich
  nichts.
- **Markdown-Vorschau eingebaut.** Eine `.md`-Änderung auszuwählen rendert das
  Dokument — Code mit Syntaxhervorhebung, eine Gliederung, die dem Scrollen
  folgt — in der Revision, in der man gerade ist, nicht nur in der Arbeitskopie.
- **Ein ganzes Diff mit angehefteter Dateiüberschrift.** Ohne Auswahl sieht man
  alle Änderungen auf einmal, und die Überschrift der Datei, die man gerade
  liest, klebt oben am Bereich, bis die Überschrift der nächsten sie wegschiebt.
- **Text markieren und kopieren, das einfach funktioniert** — kein Maus-Modus,
  kein Register, keine Tastaturakrobatik; überall im Fenster markieren und
  kopieren.
- **Jeder Bereich skalierbar, ausblendbar oder im Vollbild** — ein Layout aus
...
- **Settings** — der Einstellungsdialog ([Einstellungen](#settings)), auch über
  <kbd>Strg+,</kbd>.
- **Open Repository** — ein Verzeichnisdialog, der in einem neuen Tab öffnet
  (<kbd>Strg+O</kbd>).
...
dem Namen. Zwei Statusspalten werden gezeigt: der Zustand im Index (grün) und der
...
[gerendert](#markdown-preview), mit einem Schalter zurück zur Quelle; ein Bild
öffnet als [das Bild](#images).
...
sich jeder Git-Befehl direkt ausführen lässt. Die anderen Bereiche aktualisieren
sich automatisch, wenn sich das Repository auf der Platte ändert.
...
Tab und wird über Neustarts gemerkt; **Restore Defaults** setzt alles zurück.
...
**Word wrap**, **Diff layout** und **Markdown outline** sind dieselben Schalter,
die die Diff-Kopfzeile trägt: eine Änderung an einer Stelle wirkt an beiden.
**Word highlight** gibt es nur hier.
