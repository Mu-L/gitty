...
标题栏里的 **Send to agent** 把 index 交出去。它往右下窗格的 shell 里敲一条命令并
按下回车，就只做这些：Gitty 内部不调用任何模型，你没有送出去的东西不会离开这台机器。
agent 的提示和输出出现在终端里，那里有真正的 tty，所以 hooks 和 gpg 签名照常工作。

按钮旁的箭头用来选命令——没有对应的设置项，因为这是一个每次交接问一次、而不是每次安
装问一次的问题。下拉框列出 Gitty 记住的命令，给按钮本身运行的那条打勾，并运行你选的
那条。条目右侧的 **×** 在确认后把它移出列表——列表是唯一记录命令的地方，而菜单保持打开，所以能连着删掉几个。底部的 **New command…** 打开一个单行输入框，预填当前命令，
...
工作区窗格标题栏里的 **Search** 会在文件列表上方打开一个框，并运行 `git grep`。结果
是一份按文件分组的文档，行号在左侧；点一条会在那一行打开那个文件，并标出那一行。
...
收窄——带防抖，有清空用的 ✕——结果同样按页加载。

标题栏里的 **Graph** 在哈希旁边画出那些连线（lane）：分支在哪里岔开，merge 在哪里落
地，一个提交属于哪条线。它是从每个提交的父提交算出来的，而不是解析
