...
> divergência, prevalece o inglês. A interface em si é em inglês, então os nomes
> de botões e itens de menu ficam no original.
...
ser ocultado e trazido de volta — veja [Tela cheia e ocultar](#full-screen-and-hiding).
...
- Opcionalmente [gource](https://gource.io/) no `PATH`, para
  [a animação](#gource); nada muda se estiver ausente

## Como executar <a id="running"></a>

Instale o comando `gitty` uma vez:

```bash
npm install -g gitty-desktop      # installs the gitty command globally
```

ou, a partir de um checkout, vincule-o ao seu PATH com:
...
O caminho via `setup.sh` também instala um lançador clicável, escolhido por
plataforma.

No **Linux**, o ícone é adicionado ao tema hicolor e uma entrada `gitty.desktop`
aparece no menu de aplicativos (e no desktop, quando a sessão tem um). O cache de
ícones e o banco de dados do desktop são atualizados depois, então a entrada
aparece com seu ícone imediatamente. Ele carrega uma solução de contorno, e o app
roda com uma flag de sandbox desligada — veja
[Integração com o desktop Linux](#linux-desktop-integration).

No **macOS**, um `Gitty.app` mínimo é gravado em `~/Applications` (com um symlink
no Desktop) envolvendo o mesmo `run.sh`. Nada é empacotado: o bundle existe para
dar ao Finder e ao Dock um nome e um ícone. O Dock não é tocado — arraste para lá
você mesmo se quiser fixá-lo. Veja
[Bundle do app macOS](#macos-app-bundle).
...
- **+** ao lado dele — um seletor de diretório, abrindo o repositório que você
  escolher em uma nova aba (<kbd>Ctrl+O</kbd>). Ele fica junto ao botão do
  repositório porque os dois são o mesmo assunto: qual repositório você está
  vendo, e abrir outro.
...
- **`3 changed`** — quantos arquivos a árvore de trabalho tem sem commit, a mesma
  contagem que a linha **Working Tree** no painel de commits carrega.
...
Um *lugar* é tudo o que os dois painéis superiores estavam mostrando: a visão — a
árvore de trabalho, um commit, um intervalo de dois, um instantâneo — o arquivo
selecionado dentro dela, e o documento aberto ao lado do diff. Então uma parada
aparece como `Working tree`, `7bb7787 — Refresh screenshot batches`,
...
- **Clique direito** — remova a entrada da lista. O menu continua aberto, então
  vários podem ser limpos em sequência.
...
### Working Tree (superior esquerdo) <a id="working-tree-top-left"></a>
...
arquivos daquele commit; **Back to Work Tree** (ou <kbd>Esc</kbd>) retorna à
...
- **Inline / Side-by-Side** — uma coluna com marcadores `+`/`-`, ou antigo e novo
  lado a lado, onde uma sequência de remoções é unida às adições que a seguem.
  Metades quebradas permanecem alinhadas.
...
Manager** não é oferecido. **Back to Work Tree** (ou <kbd>Esc</kbd>) sai.
...
HTML bruto dentro do markdown não é renderizado, e links abrem no navegador do
sistema, e não dentro do app. Imagens escritas relativas ao documento são lidas do
...
segue renomeações, e clicar em um commit o abre.
...
primeira linha é a **Working Tree** — as mudanças sem commit, com uma contagem de
arquivos alterados; selecioná-la traz os painéis superiores de volta à árvore de
trabalho. Uma caixa de filtro acima do log estreita a lista para commits cuja
mensagem ou autor contém o texto que você digita — com debounce, e um ✕ para
limpar — e a lista pagina da mesma forma.
...
- **Clique direito → Open in Browser** — renderiza este commit no navegador do
  sistema; **Copy Commit URL** copia o link. Um servidor web dentro do app
  (ouvindo apenas em `127.0.0.1`, para o seu próprio navegador) serve todo
  repositório aberto como uma lista de commits navegável — o botão **Open in
  Browser** do painel de commits aterrissa lá — com os metadados, arquivos e diff
  de cada commit, e diffs por arquivo a um clique de distância. As URLs funcionam
  enquanto o repositório está aberto.
...
Se [gource](https://gource.io/) está no `PATH`, o painel de commits ganha um botão
**Gource** ao lado de **Open in Browser**: ele reproduz o histórico do repositório
...
abre uma janela própria e continua rodando depois que você fecha o Gitty; o botão
...
`PATH`, o botão simplesmente não está lá.
...
HTML](#html-preview), um blame, ou o histórico de um arquivo. Toda ocorrência é
...
| **Markdown outline** | Mostra o outline ao lado de um documento renderizado. |
...
execução. **Word wrap**, **Diff layout** e **Markdown outline** são os mesmos
...
| <kbd>Ctrl+F</kbd> | Busca no diff, no arquivo, ou no que o painel estiver mostrando |
| <kbd>Ctrl+C</kbd> / <kbd>Ctrl+Shift+C</kbd> | Copia a seleção, em qualquer lugar da janela |
| <kbd>Esc</kbd> | Volta à árvore de trabalho |
...
A entrada de desktop carrega `StartupWMClass=electron`, que é o que dá à janela em
execução seu ícone na lista de janelas e no dock. Um app Electron que é executado
em vez de empacotado reporta `electron` como sua classe de janela, qualquer que
seja o nome que a aplicação use para si mesma, então esse é o nome que a entrada
tem de corresponder — com o efeito colateral de que outro app Electron não
empacotado na mesma sessão tomaria emprestado o ícone do Gitty.

O app também roda com a sandbox SUID do Chromium desabilitada
(`ELECTRON_DISABLE_SANDBOX=1`). A correção usual — tornar `chrome-sandbox` de
propriedade do root com modo 4755 — não sobrevive dentro de `node_modules`, então
desabilitá-la é a escolha pragmática para uma ferramenta local que só lê seus
...
empacotar (electron-builder) não compraria nada além de assinatura.
